...
//  MegaPort.begin(9600);
  ...
  panelReact(motor_pin, 2);
  panelReact(loa_pin, 2);
  panelReact(LED_BLUETOOTH, 2);
  panelReact(LED_RF, 2);
...
//  Serial.println("Loop.");
  ...
  if(preBTProcess == APP_DATA_RECEIVED_SUCCESS){
    ...
    {  
      ...
      delay(1);
      tempBool = Check_RF(indTemp, APP_DATA_SEND_RF_MAX_TIMOUT);//======================> for testing
      if(!tempBool)
      {
...
//  }
 ...
 if(megasendCnt >= 200){
  ...
  if(Serial.availableForWrite()>= 63){
    Serial.print("Dang goi BTID qua Mega Pro:");
