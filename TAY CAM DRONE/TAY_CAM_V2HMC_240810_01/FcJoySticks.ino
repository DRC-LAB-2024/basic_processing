...
void JsErr(){
  ...
  while(c<n){    
    jlxErr += readJoyAnalog(JoyLeftX_PIN)/n;
    jlyErr += readJoyAnalog(JoyLeftY_PIN)/n; //analogRead(JoyLeftY);
    jrxErr += readJoyAnalog(JoyRightX_PIN)/n; //analogRead(JoyRightX);
    jryErr += readJoyAnalog(JoyRightY_PIN)/n; //analogRead(JoyRightY);
    c++;
    delay(10);
  }
  //jlxErr/=n;
...
void ReadJoySticks(){
    ...
    float jleft_x = readJoyAnalog(JoyLeftX_PIN) - 1*jlxErr;//analogRead(JoyLeftX) - 1*jlxErr;
    float jleft_y = readJoyAnalog(JoyLeftY_PIN) - 1*jlyErr;//analogRead(JoyLeftY) - 1*jlyErr;

//    Serial.print(" Zdes0: "); Serial.println(jleft_x);
//    Serial.print(" Yawdes0: "); Serial.println(jleft_y);
    
    float jright_x = readJoyAnalog(JoyRightX_PIN) - 1*jrxErr;//analogRead(JoyRightX) - jrxErr;
    float jright_y = readJoyAnalog(JoyRightY_PIN) - 1*jryErr;//analogRead(JoyRightY) - jryErr;
    ...
    //------ XU LY DX------------------------
      ...
      temp2 = (float)Dx; 
      temp2 = 0.5*temp2 + 0.5*scale_roll(temp1, pitchscale_pin);
      Dx = (long)temp2;
    //----- END OF XU LY DX------------------
    ...
    //----- XU LY DY-------------------------
     // temp1 = Dy;
      ...
      temp1 = constrain(temp1,-138,138);
      temp2 = (float)Dy;
      temp2 = 0.5*temp2 + 0.5*scale_pitch(temp1, rollscale_pin);
      Dy = (long)temp2;
     //---- END OF XU LY DY-----------------
//
    //---- XU LY Z-----------------------------------
...
float scale_roll(float temp0, unsigned int scale_pin)
{
  long temp1 = (long)temp0;
//  long scale = analogRead(scale_pin);
//  //*rscale = scale;
//  long scalevalue = map(scale, 0, 1023, 50, 138);
  long scalevalue = read_scale_value(scale_pin, rollscale_MIN, rollscale_MAX);
  
  long scalevalue_1 = map(temp1, 0, 116, 0, scalevalue);
  long scalevalue_2 = map(temp1, -111, 0, -scalevalue, 0);
  if(temp1 >= 0) return ((float)scalevalue_1);
  if(temp1 < 0) return ((float)scalevalue_2);
  else return 0;
}

float scale_pitch(float temp0,unsigned int scale_pin)
{
  long temp1 = (long)temp0;
//  long scale = analogRead(scale_pin);
//  //*pscale = scale;
...
//  long scalevalue = map(scale, 0, 1023, 50, 138);
  long scalevalue = read_scale_value(scale_pin, pitchscale_MIN, pitchscale_MAX);
//  Serial.print(" pitch212: "); Serial.println(scalevalue);
  long scalevalue_1 = map(temp1, 0, 121, 0, scalevalue);
//  Serial.print(" pitch212: "); Serial.println(scalevalue);
  long scalevalue_2 = map(temp1, -135, 0, -scalevalue, 0);
  if(temp1 >= 0) return ((float)scalevalue_1);
  if(temp1 < 0) return ((float)scalevalue_2);
  else return 0;
}

float scale_yaw(float temp0, unsigned int scale_pin)
{
  long temp1 = (long)temp0;
//  long scale = analogRead(scale_pin);
//  //*yscale = scale;
//  long scalevalue = map(scale, 0, 1023, 40, 180);
  long scalevalue = read_scale_value(scale_pin, yawscale_MIN, yawscale_MAX);
  long scalevalue_1 = map(temp1, 0, 179, 0, scalevalue);
  long scalevalue_2 = map(temp1, -179, 0, -scalevalue, 0);
  if(temp1 >= 0) return ((float)scalevalue_1);
  if(temp1 < 0) return ((float)scalevalue_2);
  else return 0;
}

float scale_Z(float temp0,unsigned int scale_pin)
{
  long temp1 = (long)temp0;
//  long scale = analogRead(scale_pin);
//  //*zscale = scale;
//  long scalevalue = map(scale, 0, 1023, 100, 5000);
  long scalevalue = read_scale_value(scale_pin, zscale_MIN, zscale_MAX);
  long scalevalue_1 = map(temp1, 0, 375, 0, scalevalue);
