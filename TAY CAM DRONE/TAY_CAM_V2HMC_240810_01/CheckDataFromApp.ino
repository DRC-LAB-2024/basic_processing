...
*/
uint8_t SplitData2Long(String data_in, long Arr[], uint16_t arrLen, char delimiter = ' '){
...
bool ReceivedDataFromESP32(uint8_t *BtMode_in, long *Rec_in){
  ...
  uint8_t dataCnt = 0, btMode_temp = *BtMode_in;
  bool readDataResult = false;
  long checksumTemp = 0;
  if(SerEsp.available()){    
    //    SerEsp.println(CheckD_in);
    if((btMode_temp == MANUALMODE_POS_SEND)||(btMode_temp==MANUALMODE_ANGLE_SEND))//Khong xu ly du lieu tu ESP32 trong mode MANUAL
    {
      return false;
    }else{
      ...
      if(Serial.availableForWrite()>=63){
        Serial.print("Data Received out: ");
      ...
      if(CheckValidUARTRecData(DataRecApp_in, UART_DATA_HEADER, UART_DATA_FOOTER)){ //Du lieu bay
        ...
        if((DataRecApp_in.startsWith("*2"))||(DataRecApp_in.startsWith("*3"))){
          ...
          Serial.println("Fly data.");
          if(Serial.availableForWrite()>=63){
          Serial.print("MAIN DATA: ");
          Serial.println(DataRecApp_in1);
          }
          dataCnt = SplitData2Long(DataRecApp_in1, Rec_in,POINT_MEMORY_INDEX_LEN_ALL, ',');
...
//            Serial.println(Rec_in[i]);
          if((dataCnt <= POINT_MEMORY_INDEX_LEN_ALL) && (dataCnt > 0)) //So du lieu nhan phu hop
          {
            ...
            if((checksumTemp == Rec_in[POINT_MEMORY_INDEX_CHECKSUM])&& (checksumTemp > 0) ) //Du lieu nhan Ok
            {
                //Cap nhat du lieu OK
